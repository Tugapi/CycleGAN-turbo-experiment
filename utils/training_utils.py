...
def parse_args_unpaired_contrastive_training():
    """
    Parses command-line arguments used for configuring an unpaired contrastive session (CUT-Turbo).
    This function sets up an argument parser to handle various training options.
    ...
    # args for the contrastive loss
    parser.add_argument("--lambda_contrastive", default=1, type=float)
    parser.add_argument("--nce_includes_all_negatives_from_minibatch", action= 'store_true', default=False,
    ...
    parser.add_argument('--nce_layers', type=str, default='2,3', help="compute NCE loss on which layers' intermediate features")
    parser.add_argument("--num_patches", type=int, default=64, help="number of patches per layer")
    parser.add_argument("--use_mlp", type=bool, default=True, help="whether to use mlp to produce vectors for computing NCE loss")
    parser.add_argument("--vector_nc", type=int, default=256, help="1D feature vector length produced by mlp")
    parser.add_argument("--init_type", type=str, default='normal', choices=['normal', 'xavier', 'kaiming', 'orthogonal'], help="initialization method of mlp")
